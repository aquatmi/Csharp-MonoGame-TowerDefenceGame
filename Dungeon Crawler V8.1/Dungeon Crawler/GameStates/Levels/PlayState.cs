...
namespace Dungeon_Crawler.GameStates
{
    ...
    public class PlayState : BaseGameState, IPlayState
    {
        ...
        void ReadLines(string level)
        {
            ...
            {
                case "EASY":
                    using (var stream = TitleContainer.OpenStream(@"Waves\Easy.txt"))
                    {
                    ...
                    break;
                case "MEDIUM":
                    using (var stream = TitleContainer.OpenStream(@"Waves\Medium.txt"))
                    {
                        using (var reader = new StreamReader(stream))
                        {
                            for (int i = 0; i < 25; i++)
                    ...
                    break;
                case "HARD":
                    using (var stream = TitleContainer.OpenStream(@"Waves\Hard.txt"))
                    {
                        using (var reader = new StreamReader(stream))
                        {
                            for (int i = 0; i < 25; i++)
